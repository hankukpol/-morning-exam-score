...
export default async function StudentReferralPage() {
        ...
        <div className="mx-auto max-w-4xl space-y-6">
          <section className="rounded-[32px] border border-ink/10 bg-white p-6 shadow-panel sm:p-8">
            ...
            <p className="mt-5 text-sm leading-8 text-slate sm:text-base">
              현재 환경에는 추천인 코드를 불러올 데이터베이스가 연결되어 있지 않습니다.
            </p>
          </section>
        </div>
      </main>
      ...
      <main className="min-h-screen bg-mist px-4 py-6 text-ink sm:px-6 lg:px-8">
        <div className="mx-auto max-w-4xl space-y-6">
          <section className="rounded-[32px] border border-ink/10 bg-white p-6 shadow-panel sm:p-8">
            ...
            <p className="mt-5 text-sm leading-8 text-slate sm:text-base">
              학생 포털에 로그인하면 내 추천인 코드와 추천 이력을 확인할 수 있습니다.
            </p>
          </section>
  ...
  // Derive the display referral code from examNumber
  const derivedCode = `REF-${viewer.examNumber}`;

  // Look up any REFERRAL DiscountCode matching this student's derived code
    ...
    <main className="min-h-screen bg-mist px-4 py-6 text-ink sm:px-6 lg:px-8">
      <div className="mx-auto max-w-4xl space-y-6">
          ...
          <div className="mt-5 flex flex-wrap items-center gap-4">
            ...
            <div className="flex-1 rounded-[20px] border-2 border-dashed border-ember/40 bg-ember/5 px-6 py-4 text-center">
              ...
              {referralCode && !referralCode.isActive && (
                <p className="mt-2 text-xs text-slate">비활성화된 코드입니다.</p>
              )}
              {referralCode?.validUntil && (
                <p className="mt-1 text-xs text-slate">
                  유효기간: {formatDate(referralCode.validUntil)}까지
                </p>
              )}
            </div>
          </div>
            ...
            <div>
              ...
              <ul className="mt-2 space-y-1.5 text-sm text-forest/80">
                <li className="flex items-start gap-2">
                  ...
                  친구가 내 코드로 수강 등록 시 포인트 혜택을 받을 수 있습니다.
                </li>
                <li className="flex items-start gap-2">
                  <span className="mt-0.5 h-1.5 w-1.5 flex-shrink-0 rounded-full bg-forest/50" />
                  자세한 혜택은 원 창구에 문의하세요.
                ...
                <li className="flex items-start gap-2">
                  <span className="mt-0.5 h-1.5 w-1.5 flex-shrink-0 rounded-full bg-forest/50" />
                  추천 코드는 수강 등록 시 직원에게 안내해 주시면 됩니다.
                </li>
              </ul>
              <p className="mt-3 text-xs text-forest/60">
                문의: 053-241-0112 (평일 09~21시 / 주말 09~18시)
              </p>
            </div>
          </div>
        </section>
        ...
        <section className="rounded-[28px] border border-ink/10 bg-white p-5 sm:p-6">
          ...
          <p className="mt-1 text-sm text-slate">내 추천 코드를 사용한 친구 목록입니다.</p>
              ...
              <table className="min-w-full divide-y divide-ink/10 text-sm">
                <thead className="bg-mist/80 text-left">
                  <tr>
                    ...
                    <th className="px-4 py-3 font-semibold text-right">수납금액</th>
                  </tr>
                </thead>
                  ...
                  {usages.map((usage) => (
                    <tr key={usage.id} className="hover:bg-mist/40">
                      ...
                      <td className="px-4 py-3 text-right">
                        {usage.payment.netAmount.toLocaleString("ko-KR")}원
                      </td>
                    </tr>
                  ))}
                </tbody>
          ...
          {usages.length > 0 && (
            <div className="mt-4 grid grid-cols-2 gap-3 sm:grid-cols-3">
              <article className="rounded-[20px] border border-ink/10 bg-mist p-4">
                <p className="text-xs text-slate">총 추천 수</p>
                <p className="mt-2 text-xl font-semibold">{usages.length}명</p>
              </article>
              {referralCode && (
                <article className="rounded-[20px] border border-ink/10 bg-mist p-4">
                  <p className="text-xs text-slate">코드 사용 횟수</p>
                  <p className="mt-2 text-xl font-semibold">{referralCode.usageCount}회</p>
                </article>
              )}
              {referralCode?.maxUsage != null && (
                <article className="rounded-[20px] border border-ink/10 bg-mist p-4">
                  <p className="text-xs text-slate">남은 사용 한도</p>
                  <p className="mt-2 text-xl font-semibold">
                    {Math.max(0, referralCode.maxUsage - referralCode.usageCount)}회
                  </p>
                </article>
              )}
            </div>
