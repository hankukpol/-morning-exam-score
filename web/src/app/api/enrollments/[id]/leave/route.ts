...
export async function POST(request: Request, context: RouteContext) {
  ...
  try {
    ...
    if (!leaveDate) {
      return NextResponse.json({ error: "휴원일을 입력해주세요." }, { status: 400 });
    }
    ...
    const result = await getPrisma().$transaction(async (tx) => {
      ...
      if (enrollment.status !== "ACTIVE") {
        throw new Error("수강 중인 학생만 휴원 처리할 수 있습니다.");
      }
    ...
    return NextResponse.json({ leaveRecord: { ...result, leaveDate: result.leaveDate.toISOString(), returnDate: null } }, { status: 201 });
  } catch (error) {
...
// PATCH /api/enrollments/[id]/leave — 복귀 처리 (leaveRecordId 필요)
export async function PATCH(request: Request, context: RouteContext) {
  ...
  try {
    ...
    if (!leaveRecordId) {
      return NextResponse.json({ error: "휴원 기록 ID가 필요합니다." }, { status: 400 });
    }
    if (!returnDate) {
      return NextResponse.json({ error: "복귀일을 입력해주세요." }, { status: 400 });
    }
    ...
    return NextResponse.json(
      { error: error instanceof Error ? error.message : "복귀 처리 실패" },
      { status: 400 },
    );
  }
}
