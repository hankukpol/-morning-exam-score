  ...
  try {
    ...
    if (typeof body.isActive !== "boolean") {
      return NextResponse.json(
        { error: "isActive 필드는 boolean이어야 합니다." },
        { status: 400 }
      );
    }
    ...
    if (notFound.length > 0) {
      return NextResponse.json(
        {
          error: `존재하지 않는 학번이 포함되어 있습니다: ${notFound.slice(0, 5).join(", ")}`,
        },
        { status: 400 }
      );
    }
    ...
    // Audit log (one entry per admin action, not per student, for performance)
