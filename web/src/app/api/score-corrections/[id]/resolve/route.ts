...
export async function PATCH(req: NextRequest, { params }: RouteContext) {
  ...
  // Verify memo exists and is a score correction
  const memo = await prisma.adminMemo.findFirst({
    where: {
      id,
      content: { contains: "[성적 오류 신고]" },
    },
    select: { id: true, status: true, relatedStudentExamNumber: true, ownerId: true },
  });
  ...
  const newStatus: AdminMemoStatus =
    action === "resolve" ? AdminMemoStatus.DONE : AdminMemoStatus.OPEN;

  // Update the original memo's status
  await prisma.adminMemo.update({
    where: { id },
    data: { status: newStatus },
  });

  // If a note was provided, create a reply memo referencing the same student
  if (note && note.trim()) {
    const prefix = action === "resolve" ? "[성적 오류 처리]" : "[성적 오류 반려]";
    await prisma.adminMemo.create({
      data: {
        title: `${prefix} 신고 #${id}에 대한 처리 메모`,
        content: `${prefix}\n\n원본 신고 ID: ${id}\n\n처리 내용:\n${note.trim()}`,
