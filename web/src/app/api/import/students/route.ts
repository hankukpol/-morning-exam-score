...
type StudentRow = {
  name: string;
  phone: string | null;
  birthDate: string | null; // YYMMDD format or null
  examType: ExamType;
...
function parseBirthDate(raw: string | null): Date | null {
  ...
  // Assume born 1900s if yy >= 30, else 2000s
  const year = yy >= 30 ? 1900 + yy : 2000 + yy;
  return new Date(year, mm - 1, dd);
}

function generateExamNumber(): string {
  const now = new Date();
...
export async function POST(request: NextRequest) {
  ...
  for (let i = 0; i < body.students.length; i++) {
    const raw = body.students[i] as Record<string, unknown>;
    ...
    const examType: ExamType =
      raw.examType === "GYEONGCHAE" ? "GYEONGCHAE" : "GONGCHAE";
    const phone =
      typeof raw.phone === "string" && raw.phone.trim() ? raw.phone.trim() : null;
    const birthDate = parseBirthDate(
      typeof raw.birthDate === "string" ? raw.birthDate : null,
    );
    ...
    try {
      if (providedExamNumber) {
        ...
        // No examNumber provided — generate one (retry if collision)
      ...
      errors.push(`행 ${i + 1} (${name}): ${msg.slice(0, 100)}`);
    }
  }
