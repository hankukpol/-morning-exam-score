...
export default async function SettingsAuditLogsPage({ searchParams }: PageProps) {
  ...
  const [totalLogs, todayCount, weekCount, filteredTotal, admins, logs] =
    await Promise.all([
  ...
  return (
    <div className="p-8 sm:p-10">
      ...
      <p className="mt-4 max-w-3xl text-sm leading-8 text-slate sm:text-base">
        모든 관리자 작업의 이력을 조회합니다. 직원별 활동 추적 및 보안 감사에 활용합니다.
      </p>
      ...
      <form
        method="GET"
        action="/admin/settings/audit-logs"
        ...
        <div className="grid gap-4 sm:grid-cols-2 lg:grid-cols-3 xl:grid-cols-5">
          ...
          <div>
            ...
            <input
              ...
              placeholder="예: AdminUser, Payment"
              className="w-full rounded-2xl border border-ink/10 bg-white px-4 py-3 text-sm"
            />
          </div>
          <div>
              ...
              <tbody>
                {logs.map((log, idx) => {
                  ...
                  return (
                    <tr
                      key={log.id}
                      className={`border-b border-ink/5 transition hover:bg-mist/60 ${idx % 2 !== 0 ? "bg-gray-50/40" : ""}`}
                    >
                      ...
                      <td className="whitespace-nowrap px-5 py-3.5 align-top">
                        <a
                          href={`/admin/settings/staff/${log.adminId}`}
                          className="inline-flex rounded-full border border-ink/10 bg-ink/5 px-2.5 py-1 text-xs font-medium text-ink hover:border-forest/30 hover:bg-forest/5 hover:text-forest"
                        >
                          {log.admin.name}
                        </a>
                      </td>
                      ...
                      <td className="whitespace-nowrap px-5 py-3.5 align-top">
                        <span
                          className={`inline-flex rounded-full border px-2.5 py-1 text-xs font-semibold ${getActionColor(log.action)}`}
                        >
