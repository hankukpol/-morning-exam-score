...
export function SystemSettingsClient({ config: initialConfig, academy: initialAcademy }: Props) {
  ...
  function handleSave() {
    startTransition(async () => {
      ...
      try {
        const [resAcademy, resSystem] = await Promise.all([
        ...
        if (!resAcademy.ok || !resSystem.ok) {
          const errData = !resAcademy.ok ? await resAcademy.json() : await resSystem.json();
      ...
      {tab === "academy" && (
        <div className="overflow-hidden rounded-[28px] border border-ink/10">
          <div className="divide-y divide-ink/10">
            {(
              [
                { key: "name" as const, label: "학원명 *", placeholder: "예: 한국경찰학원" },
                ...
                { key: "phone" as const, label: "대표 전화", placeholder: "예: 053-241-0112" },
                ...
                { key: "bankHolder" as const, label: "예금주", placeholder: "예: 한국경찰학원" },
                {
