  ...
  return (
    <div className="flex items-start gap-3">
      ...
      <div className="max-w-xs">
        <p className="mb-1 text-xs font-semibold text-ink">한국경찰학원</p>
  ...
  const getPreviewMessage = useCallback(() => {
    if (selectedType === "NOTICE") {
      return customMessage.trim()
        ? `[한국경찰학원] {studentName}님께 운영 공지를 전달드립니다.\n\n${customMessage.trim()}`
        : "[한국경찰학원] {studentName}님께 운영 공지를 전달드립니다.";
    }
    if (selectedType === "PAYMENT_COMPLETE") {
      return "[한국경찰학원] {studentName}님, 수납이 완료되었습니다.\n\n납부금액: {paymentAmount}원\n결제수단: {paymentMethod}\n\n문의: 053-241-0112";
    }
    if (selectedType === "ENROLLMENT_COMPLETE") {
      return "[한국경찰학원] {studentName}님, 수강 등록이 완료되었습니다.\n\n강좌명: {courseName}\n수강기간: {enrollmentPeriod}\n\n문의: 053-241-0112";
    }
    if (selectedType === "REFUND_COMPLETE") {
      return "[한국경찰학원] {studentName}님, 환불 처리가 완료되었습니다.\n\n환불금액: {refundAmount}원\n\n문의: 053-241-0112";
    }
    return "";
