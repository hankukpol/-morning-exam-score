...
export default async function StaffAnalyticsPage({ searchParams }: PageProps) {
  ...
  // --- Enrollment counts (CourseEnrollment.staffId = AdminUser.id) ---
  const [
    currEnrollAggs,
    prevEnrollAggs,
    currPaymentAggs,
  ...
  // --- Counseling records (counselorName is a plain string, not a FK) ---
  // We match staff name against CounselingRecord.counselorName
    ...
    <div className="p-8 sm:p-10">
      ...
      <div className="mt-5 flex flex-wrap items-end justify-between gap-4">
        <div>
          ...
          <p className="mt-4 max-w-3xl text-sm leading-8 text-slate sm:text-base">
            직원별 수강 처리 건수, 수납 총액, 상담 건수를 월별로 비교합니다.
            전월 대비 증감률을 함께 표시합니다.
          </p>
        </div>
      ...
      <div className="mt-8 grid grid-cols-1 gap-4 sm:grid-cols-3">
        ...
        <div className="rounded-[28px] border border-ink/10 bg-white p-6 shadow-sm">
          ...
          <p
            className={`mt-1 text-sm font-medium ${revenueDelta.positive ? "text-green-600" : "text-red-500"}`}
          >
        ...
        <div className="rounded-[28px] border border-ink/10 bg-white p-6 shadow-sm">
          ...
          <p
            className={`mt-1 text-sm font-medium ${enrollDelta.positive ? "text-green-600" : "text-red-500"}`}
          >
            전월 대비 {enrollDelta.text}
          </p>
          <p className="mt-0.5 text-xs text-slate">
            전월: {prevTotalEnrollments.toLocaleString("ko-KR")}건
          </p>
        </div>
        ...
        <div className="rounded-[28px] border border-ink/10 bg-white p-6 shadow-sm">
          ...
          <p
            className={`mt-1 text-sm font-medium ${counselDelta.positive ? "text-green-600" : "text-red-500"}`}
          >
            전월 대비 {counselDelta.text}
          </p>
          <p className="mt-0.5 text-xs text-slate">
            전월: {prevTotalCounseling.toLocaleString("ko-KR")}건
          </p>
        </div>
      </div>
      ...
      <div className="mt-8 grid grid-cols-1 gap-4 sm:grid-cols-3">
        ...
        <div className="rounded-[28px] border border-ember/20 bg-ember/5 p-5 shadow-sm">
          <div className="flex items-center gap-2">
            <span className="inline-flex h-7 w-7 items-center justify-center rounded-full bg-ember text-white text-xs font-bold">
              1
            </span>
          ...
          {topByRevenue && topByRevenue.totalRevenue > 0 ? (
            <>
              ...
              <p className="text-sm text-slate">
                {STAFF_ROLE_LABEL[topByRevenue.staffRole] ?? topByRevenue.staffRole}
              </p>
              <p className="mt-2 text-xl font-bold text-ember">
                {formatKRW(topByRevenue.totalRevenue)}
              </p>
            </>
          ) : (
        ...
        <div className="rounded-[28px] border border-forest/20 bg-forest/5 p-5 shadow-sm">
          <div className="flex items-center gap-2">
            <span className="inline-flex h-7 w-7 items-center justify-center rounded-full bg-forest text-white text-xs font-bold">
              1
            </span>
          ...
          {topByEnrollment && topByEnrollment.enrollmentCount > 0 ? (
            <>
              ...
              <p className="text-sm text-slate">
                {STAFF_ROLE_LABEL[topByEnrollment.staffRole] ?? topByEnrollment.staffRole}
              </p>
              <p className="mt-2 text-xl font-bold text-forest">
                {topByEnrollment.enrollmentCount.toLocaleString("ko-KR")}건
              </p>
            </>
          ) : (
            <p className="mt-3 text-sm text-slate">데이터 없음</p>
        ...
        <div className="rounded-[28px] border border-sky-200 bg-sky-50 p-5 shadow-sm">
          <div className="flex items-center gap-2">
            <span className="inline-flex h-7 w-7 items-center justify-center rounded-full bg-sky-500 text-white text-xs font-bold">
              1
            </span>
          ...
          {topByCounseling && topByCounseling.counselingCount > 0 ? (
            <>
              ...
              <p className="text-sm text-slate">
                {STAFF_ROLE_LABEL[topByCounseling.staffRole] ?? topByCounseling.staffRole}
              </p>
              <p className="mt-2 text-xl font-bold text-sky-600">
                {topByCounseling.counselingCount.toLocaleString("ko-KR")}건
              </p>
            </>
          ) : (
            <p className="mt-3 text-sm text-slate">데이터 없음</p>
          ...
          <table className="w-full text-sm">
            <thead>
              <tr className="border-b border-ink/10 bg-forest/5">
                ...
                <th className="px-4 py-3 text-right font-semibold text-forest" colSpan={2}>
                  수납 총액
                </th>
                <th className="px-4 py-3 text-right font-semibold text-forest" colSpan={2}>
                  수강 등록
                    ...
                    <tr key={row.staffId} className="transition-colors hover:bg-mist/50">
                      ...
                      <td
                        className={`px-4 py-3 text-right text-xs font-medium ${
                          revDelta.positive ? "text-green-600" : "text-red-500"
                        }`}
                      >
                        {revDelta.text}
                      </td>
                      ...
                      <td
                        className={`px-4 py-3 text-right text-xs font-medium ${
                          enrDelta.positive ? "text-green-600" : "text-red-500"
                        }`}
                      >
                        {enrDelta.text}
                      </td>
                      ...
                      <td
                        className={`px-4 py-3 text-right text-xs font-medium ${
                          couDelta.positive ? "text-green-600" : "text-red-500"
                        }`}
                      >
                        {couDelta.text}
                      </td>
                ...
                <tr className="border-t border-ink/20 bg-forest/5">
                  ...
                  <td
                    className={`px-4 py-3 text-right text-xs font-bold ${
                      revenueDelta.positive ? "text-green-600" : "text-red-500"
                    }`}
                  >
                    {revenueDelta.text}
                  </td>
                  ...
                  <td
                    className={`px-4 py-3 text-right text-xs font-bold ${
                      enrollDelta.positive ? "text-green-600" : "text-red-500"
                    }`}
                  >
                    {enrollDelta.text}
                  </td>
                  ...
                  <td
                    className={`px-4 py-3 text-right text-xs font-bold ${
                      counselDelta.positive ? "text-green-600" : "text-red-500"
                    }`}
                  >
                    {counselDelta.text}
                  </td>
