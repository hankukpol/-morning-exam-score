...
export default async function ScoreCorrectionDetailPage({ params }: Props) {
  ...
  const memo = await getPrisma().adminMemo.findFirst({
    where: {
      id,
  ...
  const student = memo.relatedStudentExamNumber
    ? await getPrisma().student.findUnique({
    ...
    <div className="p-8 sm:p-10">
      <Breadcrumbs
        items={[
          ...
          { label: "성적 오류 신고 목록", href: "/admin/score-corrections" },
          { label: "신고 상세" },
        ]}
      />

      <div className="mt-4">
        <h1 className="text-3xl font-semibold">성적 오류 신고 상세</h1>
        <p className="mt-2 text-sm text-slate">학생이 신고한 성적 오류 건의 상세 내용을 확인하고 처리할 수 있습니다.</p>
      </div>
                ...
                <Link
                  href={
                    memo.relatedStudentExamNumber
                      ? `/admin/scores/edit?examNumber=${encodeURIComponent(memo.relatedStudentExamNumber)}`
                      : "/admin/scores/edit"
                  }
