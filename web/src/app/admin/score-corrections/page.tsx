...
export default async function ScoreCorrectionsPage() {
  ...
  const corrections = await getPrisma().adminMemo.findMany({
    where: {
  ...
  const students =
    examNumbers.length > 0
      ? await getPrisma().student.findMany({
  ...
  return (
    <div className="p-8 sm:p-10">
      ...
      <div className="mt-4 flex flex-wrap items-start justify-between gap-4">
        <div>
          <h1 className="text-3xl font-semibold">성적 오류 신고 목록</h1>
          <p className="mt-2 text-sm text-slate">
            학생이 학생 포털에서 신고한 성적 오류 건입니다. 확인 후 성적 수정 화면에서 처리해 주세요.
          </p>
        </div>
        <Link
