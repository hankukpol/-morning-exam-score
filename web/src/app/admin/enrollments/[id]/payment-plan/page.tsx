...
export default async function EnrollmentPaymentPlanPage({ params }: PageProps) {
  ...
  const [enrollment, payments, settings] = await Promise.all([
    ...
    prisma.academySettings.findUnique({
      where: { id: 1 },
  ...
  const studentEnrollments = enrollment.student.courseEnrollments.map((item) => ({
    id: item.id,
  ...
  const overdueInstallmentAmount = overdueInstallments.reduce(
  ...
  const installmentProgressPercent =
    installmentRows.length > 0
      ? Math.round((paidInstallmentCount / installmentRows.length) * 100)
    ...
    <div className="min-h-screen bg-gray-100">
      ...
      <div className="no-print flex items-center justify-between gap-3 border-b bg-white px-6 py-4">
        ...
        <p className="text-sm text-gray-500">
          브라우저 인쇄 기능으로 출력하세요. 발행일 {formatDate(new Date())}
        </p>
      </div>
      ...
      <div className="flex justify-center px-4 py-8">
        <div className="plan-page w-full max-w-[920px] rounded-[28px] bg-white p-8 shadow-xl">
          <div className="border-b border-gray-200 pb-6">
            <div className="flex flex-wrap items-start justify-between gap-4">
              <div>
                ...
                <h1 className="mt-3 text-3xl font-semibold text-gray-900">수강 납부 계획서</h1>
                <p className="mt-2 text-sm text-gray-500">
                  수강 등록 기준의 수납 약정, 분납 일정, 환불 반영 내역을 함께 확인하는 문서입니다.
                </p>
              </div>
              <div className="rounded-[20px] border border-gray-200 bg-gray-50 px-4 py-3 text-sm text-gray-600">
                <div>강좌: {courseName}</div>
                <div>등록 직원: {enrollment.staff.name}</div>
                <div>원장: {settings?.directorName ?? "한국경찰학원"}</div>
              </div>
            </div>
          </div>
          ...
          <div className="mt-6 grid gap-4 md:grid-cols-4">
            ...
            <article className="rounded-[20px] border border-forest/20 bg-forest/5 p-4">
              ...
              <p className="mt-3 text-2xl font-bold text-forest">
                {formatKRW(totalScheduledAmount)}
              </p>
            </article>
            <article
            ...
            >
              ...
              <p className="mt-1 text-xs text-gray-500">
                {overdueInstallments.length > 0
                  ? `연체 ${overdueInstallments.length}건 포함`
          ...
          {installmentRows.length > 0 ? (
            <div className="mt-4 rounded-[20px] border border-gray-200 bg-gray-50 p-4">
              <div className="flex items-center justify-between text-sm">
                <span className="text-gray-600">분납 회차 진행률</span>
          ...
          <div className="mt-6 grid gap-6 lg:grid-cols-[1.1fr_0.9fr]">
            <Section
              title="학생 기본 정보"
              description="운영 절대 규칙에 따라 학번, 이름, 연락처, 수강내역을 함께 표시합니다."
            >
              <div className="grid gap-4 sm:grid-cols-2">
                <div>
                  ...
                  <Link
                    ...
                    className="mt-1 inline-flex text-base font-semibold text-forest hover:underline"
                  >
                  ...
                  <div className="mt-1 flex flex-wrap items-center gap-2">
                    ...
                    <span
                      className={`inline-flex rounded-full border px-2.5 py-1 text-xs font-semibold ${
                        ENROLLMENT_STATUS_COLOR[enrollment.status]
                      }`}
                    >
            ...
            <Section title="현재 수강 등록 정보" description="현재 출력 중인 수강 등록 한 건의 기준 정보입니다.">
              <div className="space-y-3 text-sm">
                <InfoRow label="강좌명" value={courseName} />
                ...
                <InfoRow
                  label="정가 / 할인"
                  value={`${formatKRW(enrollment.regularFee)} / ${formatKRW(enrollment.discountAmount)}`}
                />
                  ...
                  <table className="min-w-full divide-y divide-gray-200 text-sm">
                    <thead className="bg-gray-50 text-left">
                      <tr>
                        ...
                        <th className="px-4 py-3 font-semibold text-gray-600">결제수단</th>
                        ...
                        <th className="px-4 py-3 font-semibold text-gray-600">담당자</th>
                      </tr>
                    </thead>
                        ...
                        return (
                          <tr key={payment.id}>
                            <td className="whitespace-nowrap px-4 py-3 text-gray-700">
                              {formatNullableDate(payment.processedAt)}
                            </td>
          ...
          <div className="mt-6">
            <Section
              title="분납 일정"
              description="회차별 납부 예정일과 연체 여부를 함께 표시합니다."
            >
              {installmentRows.length === 0 ? (
                <div className="rounded-[20px] border border-dashed border-gray-300 px-4 py-10 text-center text-sm text-gray-500">
                  분납 회차가 등록되지 않았습니다. 현재 수강은 일시납 또는 별도 회차 계획이 없는
                  상태입니다.
                </div>
              ) : (
                <div className="overflow-x-auto">
                      ...
                      {installmentRows.map((row) => {
                        ...
                        return (
                          ...
                          >
                            ...
                            <td className="whitespace-nowrap px-4 py-3 text-gray-700">
                              {formatNullableDate(row.dueDate)}
                            </td>
                            <td className="whitespace-nowrap px-4 py-3 text-gray-700">
                              {formatNullableDate(row.paidAt)}
                            </td>
                            <td className="px-4 py-3 text-gray-700">
          ...
          <div className="mt-6 rounded-[24px] border border-gray-200 bg-gray-50 p-5 text-sm text-gray-600">
            ...
            <ul className="mt-2 space-y-1">
              <li>수납 약정 금액은 해당 수강 등록에 연결된 수납 건의 `netAmount` 기준입니다.</li>
              <li>환불 금액은 승인 또는 완료 처리된 환불 건만 반영합니다.</li>
              <li>학생명과 학번을 클릭하면 학생 상세 화면으로 이동합니다.</li>
            </ul>
          </div>
        </div>
      </div>
