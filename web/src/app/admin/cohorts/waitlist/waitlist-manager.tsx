...
export function WaitlistManager({ groups }: Props) {
  ...
  const currentGroup = groups.find((g) => g.cohortId === activeTab) ?? groups[0];
  ...
  return (
    <div className="mt-8">
      ...
      {/* Current cohort panel */}
      {currentGroup ? (
        <WaitlistPanel key={currentGroup.cohortId} group={currentGroup} />
      ) : null}
    </div>
  );
}
    ...
    <div className="mt-4 rounded-[28px] border border-ink/10 bg-white p-6">
      ...
      <div className="flex flex-wrap items-start gap-4">
        ...
        <div className="ml-auto flex flex-wrap gap-3">
          ...
          >
            <p className="text-xs text-slate">여석</p>
            <p
      ...
      {isFull && (
        <div className="mt-4 rounded-xl border border-red-200 bg-red-50 px-4 py-3 text-sm text-red-700">
          정원이 꽉 찼습니다. 수강생이 퇴원하거나 정원을 늘린 후 승격이 가능합니다.
        </div>
      )}
  ...
  return (
    <tr className="transition hover:bg-mist/20">
      ...
      <td className="px-4 py-3 font-medium text-ink whitespace-nowrap">
        <Link
          href={`/admin/students/${item.examNumber}`}
        ...
        <div className="flex flex-col gap-1">
          ...
          <div className="flex items-center gap-2">
            <button
              ...
              title={isFull ? "여석이 없어 승격할 수 없습니다" : "수강 확정으로 승격"}
            ...
            {!showCancelConfirm ? (
              ...
              <div className="flex items-center gap-1">
                <span className="text-xs text-red-600">확실히 취소?</span>
                <button
                  type="button"
