...
export default async function PeriodDetailPage({
  ...
  const activeSessions = period.sessions.filter((s) => !s.isCancelled).length;
  const lockedSessions = period.sessions.filter((s) => s.isLocked).length;

  const sessionsByExamType = {
    GONGCHAE: period.sessions.filter((s) => s.examType === "GONGCHAE"),
    GYEONGCHAE: period.sessions.filter((s) => s.examType === "GYEONGCHAE"),
  } as const;
    ...
    <div className="p-8 sm:p-10">
      ...
      <div className="mt-8 grid grid-cols-2 gap-4 sm:grid-cols-4">
        ...
        <div className="rounded-[28px] border border-ink/10 bg-white px-6 py-5">
          ...
          <p className="mt-1 text-xs text-slate">채점 잠금됨</p>
        </div>
        <div className="rounded-[28px] border border-ink/10 bg-white px-6 py-5">
        ...
        return (
          <div key={examType} className="mt-6">
            <h2 className="text-lg font-semibold text-ink">
              ...
              <span className="ml-2 text-sm font-normal text-slate">({sessions.length}건)</span>
            </h2>
            ...
            {sessions.length === 0 ? (
              ...
              <div className="mt-4 overflow-hidden rounded-[28px] border border-ink/10 bg-white">
                <table className="min-w-full divide-y divide-ink/10 text-sm">
                  <thead>
                    <tr>
                      {["주차", "과목", "시험일", "상태", "취소 사유", "성적 입력"].map((h) => (
                        <th
                          key={h}
