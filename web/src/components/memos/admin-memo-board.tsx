        ...
        <div className="rounded-[28px] border border-ink/10 bg-white p-6">
          <div className="flex flex-wrap items-start justify-between gap-4">
            ...
            {editingId ? (
              <button
                type="button"
                onClick={resetForm}
                className="inline-flex items-center rounded-full border border-ink/10 px-4 py-2 text-sm font-semibold text-ink transition hover:border-ink/30"
              >
                새 메모로 전환
              </button>
            ) : null}
          ...
          <div className="mt-6 space-y-4">
            ...
            <div className="flex flex-wrap gap-3 pt-2">
              <button type="button" onClick={saveMemo} disabled={isPending} className="inline-flex items-center rounded-full bg-ink px-5 py-3 text-sm font-semibold text-white transition hover:bg-forest disabled:cursor-not-allowed disabled:opacity-60">
                {editingId ? "메모 저장" : "메모 등록"}
              </button>
              <button type="button" onClick={resetForm} disabled={isPending} className="inline-flex items-center rounded-full border border-ink/10 px-5 py-3 text-sm font-semibold text-ink transition hover:border-ink/30 disabled:cursor-not-allowed disabled:opacity-60">
                입력 초기화
              </button>
            </div>
          </div>
                    ...
                    return (
                      <article key={memo.id} className={`rounded-[24px] border p-5 shadow-sm ${COLOR_STYLES[memo.color]}`}>
                        ...
                        <div className="mt-5 flex flex-wrap gap-2">
                          <button type="button" onClick={() => { setEditingId(memo.id); setForm(buildFormState(memo)); }} disabled={isPending || !editable} className="inline-flex items-center rounded-full border border-ink/10 bg-white/80 px-4 py-2 text-sm font-semibold text-ink transition hover:border-ink/30 disabled:cursor-not-allowed disabled:opacity-50">편집</button>
