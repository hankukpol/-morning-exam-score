...
function MonthlyBarChart({ stats }: { stats: MonthlyStat[] }) {
  ...
  const maxVal = Math.max(...stats.flatMap((s) => [s.earned, s.spent]), 1);
...
export function PointsHistoryPanel({
  ...
  return (
    <div className="space-y-6">
      ...
      <section className="rounded-[28px] border border-ink/10 bg-white p-5 sm:p-6">
        <div className="flex flex-wrap items-start justify-between gap-3">
          <div>
            ...
            <p className="mt-2 text-sm leading-7 text-slate">최근 6개월 적립·사용 현황입니다.</p>
          </div>
        </div>
      ...
      {/* 포인트 사용처 안내 */}
      <section className="rounded-[28px] border border-ink/10 bg-white p-5 sm:p-6">
        <h2 className="text-xl font-semibold">포인트 사용처 안내</h2>
        <p className="mt-2 text-sm leading-7 text-slate">
          적립된 포인트는 아래 항목에 사용할 수 있습니다. 창구에 문의해 주세요.
        </p>
        <div className="mt-4 grid gap-3 sm:grid-cols-2">
          {[
            { icon: "📚", label: "단과 수업 수강료", desc: "단과 강좌 수강 시 포인트 차감 적용" },
            { icon: "🏫", label: "시설 이용", desc: "스터디룸·자습실 예약 시 포인트 적용" },
            { icon: "📖", label: "교재 구입", desc: "원내 판매 교재 구입 시 포인트 사용 가능" },
            { icon: "🎁", label: "특별 이벤트", desc: "학원 이벤트·경품 응모 시 포인트 소모" },
          ].map(({ icon, label, desc }) => (
            <div
              key={label}
        ...
        <p className="mt-4 rounded-2xl border border-ember/20 bg-ember/5 px-4 py-3 text-xs leading-6 text-ember">
          포인트는 관리자가 직접 지급하며, 만료 없이 유지됩니다. 사용 시 창구에 직접 문의해 주세요.
        </p>
      </section>
      ...
      <section className="rounded-[28px] border border-ink/10 bg-white p-5 sm:p-6">
        <div className="flex flex-wrap items-start justify-between gap-4">
          <div>
            ...
            <p className="mt-2 text-sm leading-7 text-slate">적립·사용 이력을 유형별로 확인하세요.</p>
          </div>
          <div className="rounded-[20px] border border-ink/10 bg-mist px-4 py-2.5 text-sm font-bold text-ink">
            잔액 {formatPoint(totalPoints)}
          </div>
        </div>
              ...
              return (
                ...
                >
                  <div className="flex flex-wrap items-start justify-between gap-3">
                    ...
                    <div
                      className={`text-xl font-bold ${isSpent ? "text-red-600" : "text-ember"}`}
                    >
                      {isSpent ? "" : "+"}{formatPoint(log.amount)}
                    </div>
                  </div>
                  <div className="mt-3 flex flex-wrap items-center justify-between gap-2">
                    <p className="text-sm leading-relaxed text-ink">{log.reason}</p>
                    <p className="text-xs text-slate">{formatDateTime(log.grantedAt instanceof Date ? log.grantedAt.toISOString() : log.grantedAt)}</p>
                  </div>
                </article>
              );
            })}
